...
class AdminLoginView(View):
    @docs(
        tags=["admin"], summary="Зайти как админ", description="Зайти как админ"
    )
...
class AdminCurrentView(View):
    @docs(
        tags=["admin"],
        summary="Получить информацию о текущем админе",
        description="Получиить информацию об админе",
    )
    @response_schema(OkResponseSchema, 200)
    async def get(self):
        try:
...
class AdminSessionsView(View):
    @docs(
        ...
        description="Все игровые сессии",
    )
    ...
    async def get(self):
        try:
            ...
            for session in sessions:
                players_count = (
                    await self.store.admin_accessor.get_session_players_count(
                        session.id
                    )
                )
            ...
            return json_response(
                data={"amount": len(sessions_data), "sessions": sessions_data}
            )
...
class AdminPlayersView(View):
    ...
    async def get(self):
        try:
            ...
            players_data = [
                {
                    ...
                    "username": (
                        player.username or f"player_{player.telegram_id}"
                    ),
            ...
            return json_response(
                data={"amount": len(players_data), "players": players_data}
            )
...
class AdminQuestionsView(View):
    @docs(
        ...
        description="Посмотреть список вопросов",
    )
    ...
    async def post(self):
        try:
            ...
            if not question:
                return error_json_response(
                    400, "bad_request", "Failed to create question"
                )
...
class AdminQuestionDetailView(View):
    @docs(
        tags=["admin"],
        summary="Обновить статус вопроса",
        description="Обновить статус вопроса",
    )
    async def patch(self):
        try:
            ...
            if is_active is None:
                return error_json_response(
                    400, "bad_request", "is_active field required"
                )
